 ...
 *
 * stores a cbor array when C_X_len > 1
 * stores a cbor unsigned/negative integer - 24 when C_X_len = 1 and
 *  identifier < 48 (i.e. 0 - 47 are mapped to -24 - 23 to maintain a
 * single byte (draft-ietf-lake-edhoc-12 3.3)
 *
 ...
 */
static size_t
edhoc_compose_info(uint8_t *buf, size_t *buf_size, int16_t alg, uint8_t *enc, size_t enc_len, char *ident, size_t ident_len ,size_t length){
  uint8_t *pt = buf;
  size_t nr = 0;
 ...
 */
static int
edhoc_gen_MAC_2(edhoc_ctx_t *edhoc_ctx, char IR, coap_bin_const_t *TH_2, uint8_t *prk_x, uint8_t *MAC_2){
                        ...
                        coap_bin_const_t *message_1)
{
  ...
  if (ok != 0 || mm < 0 || mm >= EDHOC_METHOD_LAST) {
    coap_log(LOG_ERR, "EDHOC message_1 (method) is corrupted\n");
    return -2;
  }
                        ...
                        coap_bin_const_t *message_2)
{
  ...
  *C_R_len = 0;
  ...
  ok = oscore_cbor_get_string_array(&buf, &key, &key_len);
  if(ok != 0){
    coap_log(LOG_ERR,  "EDHOC message_2 is corrupted\n");
    return -2;
  } /* if ok */
                        ...
                        coap_binary_t *message_1)
{
  ...
  if (ok == 0)
    session->edhoc_message_4 = edhoc_create_message_4(session, hkdf_alg, CIPHER3, CIPHER3_len, message_1, message_2);
  coap_free(CIPHER3);
  if (ok != 0)return NULL;
 ...
 *     C_R : bstr / int,            # variable length connection identifier
 *   )
 *
 * Updates edhoc->
 *   G_Y_private_key
...
edhoc_create_message_2(edhoc_ctx_t *edhoc_ctx, coap_bin_const_t *message_1)
{
  ...
  uint8_t CIPHERTEXT_2[MAX_TAG_LEN];
  edhoc_suite_t *suite = &supported_cipher_suites[edhoc_ctx->selected_suite];
  coap_binary_t *message_2 = NULL;
  coap_bin_const_t *G_RX = NULL;
  coap_binary_t *I_CRED_R = NULL;
  ...
  if (edhoc_ctx->method == EDHOC_METHOD_I_SIG_R_DH ||
    ...
    oscore_cbor_put_map(&pt, &pt_len, 4);
    ...
    if (combine == NULL)
      goto exit1;
    memcpy(combine->s, I_CRED_R->s, I_CRED_R->length);
    memcpy(&combine->s[I_CRED_R->length], CRED_R->s, CRED_R->length);
    MAC_2_info = coap_new_binary(3 + 8 + TH_2->length + combine->length);
    if (MAC_2_info == NULL)
      goto exit1;
    ...
    oscore_cbor_put_text(&pt, &pt_len, "MAC_2", 5);
    ...
    KEYSTREAM_2_info = coap_new_binary(3 + 4 + 11 + TH_2->length);
 ...
 *     C_I : bstr / int,            # variable length connection identifier
...
edhoc_create_message_1(edhoc_ctx_t *edhoc_ctx)
{
  ...
  size_t data_len = message_1->length;
                            ...
                            coap_oscore_conf_t *oscore_conf)
{
  ...
  edhoc_ctx->C_I = oscore_conf->recipient_id[0];
